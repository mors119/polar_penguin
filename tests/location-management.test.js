...
function createContext(rows) {
  const headers = ['상품품목코드', '상품명', '옵션명', '가용재고', '예약재고', '기본보관위치'];
  ...
  context.COL = { 상품품목코드: '상품품목코드', 상품명: '상품명', 옵션명: '옵션명',
    가용재고: '가용재고', 예약재고: '예약재고', 기본보관위치: '기본보관위치' };
...
test('location save trims input, updates only the location column, and reports invalid SKU', () => {
  ...
  const beforeOtherColumns = master.rows.map(row => row.slice(0, 5));
  ...
  assert.equal(master.rows[1][5], 'B-02-03');
  assert.deepEqual(master.rows.map(row => row.slice(0, 5)), beforeOtherColumns);
});

...
test('order flow reuses S2-S4 and creates a PDF only for a newly created instruction', () => {
  const calls = [];
  context.S2_1_주문CSV취입 = () => { calls.push('S2'); return { 신규: 1 }; };
  ...
  assert.deepEqual(calls, ['S2', 'S3', 'S4', 'S9']);
});

test('inventory flow synchronizes stock then re-evaluates reserved orders before picking', () => {
  const calls = [];

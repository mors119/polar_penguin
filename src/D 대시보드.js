...
function collectOrderStatus_() {
  var out = { 처리완료: 0, 예약: 0, 출고완료: 0, 취소: 0, 전체: 0,
    예약전체: 0, 예약출고가능: 0, 예약부족: 0, 예약목록: [],
    피킹: { 속도: 0, 예상분: null }, 권고: { 제목: '정상 운영', 내용: ['Input과 피킹 결과는 자동 처리됩니다.'], 색: DASHCOLOR.좋음 } };
  ...
  out.예약전체 = reservation.전체; out.예약출고가능 = reservation.출고가능; out.예약부족 = reservation.재고부족;
  out.예약목록 = reservation.주문;
  if (out.예약출고가능) out.권고 = { 제목: '예약 주문 release 가능', 내용: [out.예약출고가능 + '건을 운영자가 선택해 피킹서를 생성할 수 있습니다.'], 색: DASHCOLOR.경고 };
  return out;
}
...
function collectStockStatus_() {
  ...
  var reservation = collectPreorderData_(), sku = {};
  reservation.주문.forEach(function (order) { order.부족.forEach(function (item) { sku[item.코드] = (sku[item.코드] || 0) + item.부족; }); });
  out.예약부족SKU = Object.keys(sku).map(function (code) { return { 코드: code, 부족: sku[code] }; });
  return out;
}
...
function collectPickingStatus_() {
  ...
  lines.rows.forEach(function (row) {
    ...
    if (state === ENUM.라인상태.완료) out.처리라인++;
  });
...
function renderIntegratedDashboard_(ss, order, stock, picking, operation) {
  ...
  dashboardSection_(sh, 9, '주문', [['처리완료', order.처리완료], ['예약', order.예약], ['출고완료', order.출고완료], ['취소', order.취소], ['전체', order.전체]]);
  dashboardSection_(sh, 14, '예약', [['전체', order.예약전체], ['지금 출고 가능', order.예약출고가능], ['재고 부족', order.예약부족]]);
  dashboardSection_(sh, 19, '피킹', [['작업 대상 주문', picking.작업대상주문], ['미처리 품목', picking.미처리라인], ['처리 완료 품목', picking.처리라인], ['진행률', picking.진행률 + '%'], ['출력 오류', picking.kpi.출력오류]]);
  ...
  sh.getRange(36, 1, 1, 8).setValues([['파일 입력: Drive Input', '', 'Input 지금 처리', '', '예약 주문 피킹서 생성', '', '피킹 결과 지금 반영', '']]).setFontWeight('bold');

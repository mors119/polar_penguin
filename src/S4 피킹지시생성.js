 ...
 * ============================================================
 *  v4 변경
 *   · 담당자 자동 배정 제거 — 헤더의 피킹담당자는 공란으로 둔다.
 *     작업자가 직접 이름을 적거나, 출력 양식(S9)에서 이름을 넣으면 채워진다.
 *   · 카트 슬롯을 지시마다 1로 리셋하지 않고 당일 연속 번호로 부여한다.
 *     같은 날 슬롯 1이 두 개 생기는 혼선을 막는다.
 *   · 신규 행을 시트 최상단에 삽입한다.
 *
 *  피킹지시번호의 역할
 *   이제는 작업 단위가 아니라 「재발행 방지용 배치 도장」이다.
 *   실제 작업 단위는 작업자가 출력하는 지시서(S9)다.
 * ============================================================
 */
...
function S4_1_피킹지시생성() {
  return withLock_(function () {
    ...
    if (!대상.length) {
      alert_('피킹할 주문이 없습니다.\n\n확정된 주문만 대상입니다. 먼저 「S3. 주문 확정」을 실행하세요.');
      return { 생성: false };
    }
    ...
    var 시작슬롯 = nextSlotNo_(헤더, h.지시번호, h.슬롯);   // ★ 당일 연속
...
function buildMasterMap_() {
  ...
  for (var i = 0; i < m.rows.length; i++) {
    ...
    map[code] = {
      ...
      이미지: c.이미지 >= 0 ? toStr_(m.rows[i][c.이미지]) : '',

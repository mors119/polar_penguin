 ...
 *  부분 출고를 하지 않기 때문이다.
 *
 *  예약대기 주문은 매 실행마다 다시 검증한다.
 *  다만 상품마스터의 예약상품이 Y인 동안은 재고와 무관하게 대기하며,
 *  예약상품 표시가 해제된 뒤 가용재고가 충분해야 확정으로 전환된다.
 * ============================================================
 */

/**
 * 접수/예약대기 주문을 주문번호 단위로 확정, 예약대기, 취소 중 하나로 판정한다.
 * @return {Object} 판정별 건수와 주문 목록
 * @sideEffect 확정 주문의 수량을 가용재고에서 예약재고로 이동하고 주문/재고이동로그를 갱신
 */
function S3_1_주문확정() {
  return withLock_(function () {
    ...
    순서.forEach(function (no) {
      ...
      Object.keys(필요).forEach(function (code) {
        ...
        // 예약상품을 하나라도 포함하면 부분 출고하지 않고 주문 전체를 대기시키므로 여기서는 재고 부족으로 취소하지 않는다.
        if (예약구분[code]) {
      ...
      // ---------- 판정 3: 확정 및 재고 확보 ----------
      // 확정 시점에 실물을 차감하지 않고 가용재고 → 예약재고로 이동한다.
      // 실제 출고 소진은 피킹 결과가 O로 확인된 뒤 S5가 처리한다.
      Object.keys(필요).forEach(function (code) {
        var mi = 행번호[code];
        가용[mi] -= 필요[code];
        예약재고[mi] += 필요[code];
